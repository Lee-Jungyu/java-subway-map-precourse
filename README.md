...
- 지하철 노선 관련 기능
  - 지하철 노선 등록
    - 지하철 노선 이름 중복 등록 불가
    - 지하철 노선 이름 최소 2글자 이상
    - 노선 등록 시 상행 종점역과 하행 종점역 이름 입력 받음
      - 존재하지 역일 경우 에러
  - 지하철 노선 삭제
    - 존재하지 않는 지하철 노선 삭제 불가
  - 지하철 노선 조회
    - 등록된 모든 지하철 노선 조회
    - 출력 형식: [INFO] 지하철 노선 이름
- 지하철 구간 관련 기능
  - 지하철 구간: 역과 역 사이를 구간이라 하고 이 구간들의 모음이 노선
  - 지하철 구간 추가
    - 역과 역 사이에 새로운 역 추가 가능
    - 지하철 노선, 지하철 역, 구간 번호 입력
    - 존재하는 지하철 역과 지하철 노선에만 추가 가능
    - 단 노선에서 갈래길은 불가
  - 지하철 구간 삭제
    - 노선에 등록되지 않은 역 삭제 불가
    - 종점을 삭제할 경우 다음 역을 종점으로 설정
    - 노선에 포함된 역이 2개 이하일 경우 제거 불가
- 지하철 노선에 등록된 역 조회 기능
  - 노선의 상행 종점부터 하행 종점까지 연결된 순서대로 역 목록 조회
- 초기 설정
  - 지하철 역으로 교대역, 강남역, 역삼역, 남부터미널역, 양재역, 양재시민의숲역, 매봉역을 우선 등록
  - 지하철 노슨으로 2호선, 3호선, 신분당선을 우선 등록
  - 각 노선 당 역을 다음과 같이 우선 등록
    - 2호선: 교대역 - 강남역 - 역삼역
    - 3호선: 교대역 - 남부터미널역 - 양재역 - 매봉역
    - 신분당선: 강남역 - 양재역 - 양재시민의숲역
- 에러 출력
